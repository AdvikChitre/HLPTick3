...
import { length, empty as empty_1, tryLast, append, singleton, indexed, fold, item, collect, sortBy, ofArray } from "../fable_modules/fable-library.4.1.4/List.js";
import { defaultOf, int32ToString, comparePrimitives } from "../fable_modules/fable-library.4.1.4/Util.js";
import { split, printf, toText } from "../fable_modules/fable-library.4.1.4/String.js";
...
import { add, tryFind, empty } from "../fable_modules/fable-library.4.1.4/Map.js";
...
export function getErrorTable(errorList, addButton) {
    let children_18, props_18, props_20, children_26, children_24, props_24, children_32, props_32, props_34;
    const getSuggestionLine = (suggestions, replaceType, line, col) => {
        let props_2;
    ...
    const getErrorTableLine = (index, extraMessage, line_2, col_1) => {
        ...
        if (copyable) {
            const suggestions_1 = split(text, ["|"], void 0, 1);
            return singleton((children_8 = [(props_6 = [["style", {
                color: "Black",
                verticalAlign: "Middle",
                whiteSpace: "pre",
            }]], react.createElement("td", keyValueList(props_6, 1), showLine)), getSuggestionLine(suggestions_1, extraMessage.Replace, line_2, col_1)], react.createElement("tr", {}, ...children_8)));
        }
        else {
    ...
    const getErrorTableLines = (error) => {
        ...
        if (error.ExtraErrors == null) {
            ...
            const tLine = collect((tupledArg) => {
                ...
                return getErrorTableLine(index_1, mess, line_3, error.Col);
    ...
    if (length(errorList) === 0) {
        const tLine_1 = singleton((children_32 = [(props_32 = [["style", {
            color: "Black",
            verticalAlign: "Middle",
            whiteSpace: "pre",
        }]], react.createElement("td", keyValueList(props_32, 1), " ")), (props_34 = [["style", {
            color: "Black",
            whiteSpace: "pre-wrap",
        }]], react.createElement("td", keyValueList(props_34, 1), "No errors"))], react.createElement("tr", {}, ...children_32)));
        tableLines = singleton(react.createElement("tbody", {}, ...tLine_1));
    }
    else {
        tableLines = collect((err_1) => singleton(getErrorTableLines(err_1)), sortBy((err) => err.Line, errorList, {
            Compare: comparePrimitives,
        }));
    }
    const tableChildren = append(tableFormat, tableLines);
    if (length(tableLines) !== 0) {
        const props_40 = [["style", {
            fontSize: "16px",
            tableLayout: "Fixed",
            width: "100%",
            borderRight: "groove",
        ...
        return react.createElement("table", keyValueList(props_40, 1), ...tableChildren);
    }
    else {
