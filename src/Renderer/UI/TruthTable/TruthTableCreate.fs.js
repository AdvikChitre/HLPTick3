import { zip, singleton as singleton_1, item, mapFold, contains, partition, isEmpty, length, fold as fold_1, filter, map, append } from "../../fable_modules/fable-library.4.1.4/List.js";
import { TruthTable, TruthTableCell, CellData, ConstraintSet, CellIO } from "../../Simulator/TruthTableTypes.fs.js";
import { FSharpMap__get_Count, ofList, toList } from "../../fable_modules/fable-library.4.1.4/Map.js";
import { truncate, mapIndexed, toList as toList_1, initialize, empty, fold, singleton, append as append_1, map as map_1, collect, delay } from "../../fable_modules/fable-library.4.1.4/Seq.js";
import { compare, arrayHash, equalArrays, equals } from "../../fable_modules/fable-library.4.1.4/Util.js";
...
import { buildFastSimulationFData, extractViewers, extractFastSimulationIOsFData, changeInputBatch } from "../../Simulator/Fast/FastRun.fs.js";
 ...
 */
export function tableLHS(inputs, ttt) {
    const rowLimit = ~~Math.pow(2, ttt.BitLimit) | 0;
    const inputValuesSeq = (count, io) => {
        const w = io[2] | 0;
        const seqLength = ~~Math.pow(2, w) | 0;
        const matchValue = getConstraintsOnIO(new CellIO(0, [io]), ttt.InputConstraints);
    ...
    const patternInput = partition((io_1) => contains(io_1, ttt.AlgebraIns, {
        Equals: equalArrays,
    ...
    const numericRows = toList_1(map_1((l) => toList_1(mapIndexed((i, n) => {
        ...
        return new TruthTableCell(new CellIO(0, [item(i, numericInputs)]), new CellData(0, [convertIntToWireData(w_1, toInt64(fromInt32(n)))]));
...
export function simulateInputCombination(rowLHS, outputs, simData) {
    ...
    }, extractFastSimulationIOsFData(outputs, simData));
 ...
 */
export function truthTable(simData, ttType, isRegeneration) {
    ...
    if (isRegeneration) {
        ...
        const matchValue = buildFastSimulationFData(2, "", simData.Graph);
        if (matchValue.tag === 0) {
    ...
    const inputs = map((tuple) => tuple[0], extractFastSimulationIOsFData(simData.Inputs, tableSimData));
    const outputs = map((tuple_1) => tuple_1[0], extractFastSimulationIOsFData(simData.Outputs, tableSimData));
    const viewers = extractViewers(simData);
    const patternInput = tableLHS(inputs, ttType);
