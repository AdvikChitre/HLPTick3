...
//--------------------------------------------------------------------------------------------------//
    ...
    /// Sample data for rotations
    let rotationSamples =
        [Rotation.Degree0; Rotation.Degree90; Rotation.Degree180; Rotation.Degree270]
        |> fromList

    /// Sample data for flips
    // Vertical flip = horizontal flip with 180 rotation
    let flipSamples =
        [Some SymbolT.FlipType.FlipHorizontal; None]
        |> fromList


    /// Sample data giving a 2D grid
    let gridPositions =
        (horizLinePositions, vertLinePositions)
        ||> GenerateData.product (fun (pos1: XYPos) (pos2: XYPos) -> {X=pos1.X; Y=pos2.Y})

    /// True if no overlap with square
    let isNoOverlap radius (pos: XYPos) =
        pos.X < (middleOfSheet.X - radius)
        || pos.X > (middleOfSheet.X + radius)
        || pos.Y < (middleOfSheet.Y - radius)
        || pos.Y > (middleOfSheet.Y + radius)

    /// Sample data of 2D grid with component-component overlap filtered out
    let gridPositionsNoOverlap radius =
        gridPositions
        |> GenerateData.filter (isNoOverlap radius)

    /// Sample data combining vertical and horizontal to give a 2D grid
    let RandPosWithRotationFlip =
        (rotationSamples, gridPositionsNoOverlap 40.)
        ||> product (fun rot pos -> (rot, pos))
        |> product (fun flip (rot, pos) -> (flip, rot, pos)) flipSamples
        |> toArray
        |> shuffleA
        |> fromArray
    ...
    let makeTest7Circuit ((flip), (rotation), (andPos:XYPos)) =
        initSheetModel
        |> placeSymbol "G1" (GateN(And,2)) andPos
        |> Result.bind (placeSymbol "FF1" DFF middleOfSheet)
        |> Result.bind (placeWire (portOf "G1" 0) (portOf "FF1" 0))
        ...
        |> getOkOrFail
        |> match flip with
            | Some direction -> flipSymbol "G1" direction
            | None -> id
        |> rotateSymbol "G1" rotation
...
//---------------------------------------------------------------------------------------//
    ...
    module Tests =
        ...
        let test5 testNum firstSample dispatch =
            runTestOnSheets
                "2D-positioned AND + DFF: fail all tests"
                firstSample
                gridPositions
                makeTest1Circuit
        ...
        let test6 testNum firstSample dispatch =
            runTestOnSheets
                "2D-positioned no-overlap AND + DFF: fail all tests"
                firstSample
                (gridPositionsNoOverlap 40.)
                makeTest1Circuit
                Asserts.failOnWireIntersectsSymbol
                dispatch
            |> recordPositionInTest testNum dispatch
